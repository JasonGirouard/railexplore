...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        error_message = "Usage: python path_calculation.py <origin_station> <destination_station>"
    ...
    if not origin_station or not destination_station:
        error_message = "Origin and destination stations are required"
        logging.error(error_message)
        print(json.dumps({"error": error_message}))
        sys.exit(1)

    if origin_station not in df['from_station'].unique() or destination_station not in df['to_station'].unique():
        error_message = "Invalid origin or destination station"
        logging.error(error_message)
        print(json.dumps({"error": error_message}))
        sys.exit(1)
